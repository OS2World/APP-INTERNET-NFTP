...
int nls_init (char *system_libpath, char *user_libpath)
{
    ...
    bindtextdomain ("nftp", "/usr/local/share/locale");
    textdomain ("nftp");
    bind_textdomain_codeset ("nftp", "KOI8-R");
...
void usage (void)
{
    fprintf (stderr,
             ...
             M("connect to URL"),
             M("download (G,g) or upload (P,p) file-URL"),
             M("process transfers programmed in cmdfile.txt"),
             M("look for 'word' in bookmarks(b) or history(h)"),
             M("URL should be in the form\n"
...
void config_fly (void)
{
    ...
    {
        fl_sym.h = M("-")[0];      //nls_strings[M_HBAR][0];
        fl_sym.v = M("|")[0];      //nls_strings[M_VBAR][0];
        fl_sym.x = M("+")[0];      //nls_strings[M_CROSS][0];
        fl_sym.fill1 = M("*")[0];  //nls_strings[M_FILL1][0];
        fl_sym.fill2 = M("#")[0];  //nls_strings[M_FILL2][0];
        fl_sym.fill3 = M("%")[0];  //nls_strings[M_FILL3][0];
        fl_sym.fill4 = M("@")[0];  //nls_strings[M_FILL4][0];
        fl_sym.c_lu = M("/")[0]; //nls_strings[M_LU][0];
        fl_sym.c_ru = M("\\")[0]; //nls_strings[M_RU][0];
        fl_sym.c_ld = M("\\")[0]; //nls_strings[M_LD][0];
        fl_sym.c_rd = M("/")[0]; //nls_strings[M_RD][0];
        fl_sym.t_l  = M("+")[0]; //nls_strings[M_LT][0];
        fl_sym.t_r  = M("+")[0]; //nls_strings[M_RT][0];
        fl_sym.t_u  = M("+")[0]; //nls_strings[M_UT][0];
        fl_sym.t_d  = M("+")[0]; //nls_strings[M_DT][0];
        fl_sym.placeholder = M(".")[0]; //nls_strings[M_PLACEHOLDER][0];
        fl_sym.marksign    = M("!")[0]; //nls_strings[M_MARKSIGN][0];
    }
