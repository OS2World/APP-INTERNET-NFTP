...
{
    char *buf, *p, *savbuf, fname[MAXPATHLEN];
    static char searchbuf[128] = "";
    int  i, fl, cl, ll, pg, fp, mp, spg, key, line, run=TRUE, buflen;
    int  c1, c2, r1, r2, enc;
    FILE *fh;
    ...
    /* find out the longest line length */
    mp = 80;
    for (i=0; i<nl; i++)
    {
        mp = max1 (mp, strlen (s[i]));
    }
    ...
    do
    {
        ...
        // redraw screen
        buflen = video_hsize() + 1;
        buf = xmalloc (buflen);
        memset (buf, ' ', buflen);
        snprintf1 (buf, buflen, " %c %d/%d %+d ", fl_sym.v,
                   min1 (fl+pg, nl), nl, fp);
        ...
        for (line=r1; line <= r2; line++)
        {
            ...
            buf[c2-c1+1] = '\0';
            p = str_stristr (buf, searchbuf);
            if (options.guess_cyrillic && enc != EN_UNKNOWN)
        ...
        if (IS_KEY(key))
        {
            ...
            case _F7:
            case _CtrlF:
                if (entryfield (M("Enter string to search:"),
                                searchbuf, sizeof(searchbuf), 0) == PRESSED_ESC)
                {
                    break;
                }
            ...
            case _CtrlW:
                fname[0] = '\0';
                if (entryfield (M("Enter filename to save into:"),
                                fname, sizeof(fname), 0) == PRESSED_ESC)
                {
                    break;
                }
                fh = fopen (fname, "w");
                if (fh == NULL)
                {
                    fly_ask_ok (ASK_WARN, M("Cannot open file %s"), fname);
                    break;
                }
                for (i=0; i<nl; i++)
                {
                ...
                fclose (fh);
                fly_ask_ok (0, M ("File has been saved"));
                break;
...
char *mon2txt (int month)
{
    switch (month)
    {
        case 0:  return M ("Jan"); break;
        case 1:  return M ("Feb"); break;
        case 2:  return M ("Mar"); break;
        case 3:  return M ("Apr"); break;
        case 4:  return M ("May"); break;
        case 5:  return M ("Jun"); break;
        case 6:  return M ("Jul"); break;
        case 7:  return M ("Aug"); break;
        case 8:  return M ("Sep"); break;
        case 9:  return M ("Oct"); break;
        case 10: return M ("Nov"); break;
        case 11: return M ("Dec"); break;
    }
    return "???";
}
...
void choose_local_drive (void)
{
    ...
    do
    {
        ...
        video_put (M("Choose drive"), r1+1, c1 + (c2-c1-12)/2);
        for (i=0; i<ndrives; i++)
        {
            ch = 'A'+drives[i];
...
int save_listing (char *filename)
{
    ...
    if (fh == NULL)
    {
        fly_ask_ok (ASK_WARN, M("Cannot open file %s"), filename);
        return -1;
    }

    url = compose_url (site.u, "", 0, 0);
    fprintf (fh, M ("Server    : %s\n"
                    "Directory : %s\n"), site.u.hostname, RCURDIR.name);
    fprintf (fh, "URL: %s\n\n", url);
    ...
    fly_ask_ok (0, M("Listing has been saved"));
    return 0;
}
...
void set_local_path (char *path)
{
    ...
    if (path[1] == ':')
    {
        ...
        if (newdrive < 26  && newdrive >= 0)
        {
            if (change_drive (newdrive) ||
                getcwd (buffer, sizeof (buffer)) == NULL)
            {
                fly_ask_ok (ASK_WARN, M ("Can't access drive %c:"), path[0]);
                return;
            }
        }
    ...
    if (path[0] == '\0') return;
    if (chdir (path))
        fly_ask_ok (ASK_WARN, M ("Directory '%s' does not exist"), path);
}
...
void browse_buffer (char *s, int len, char *title)
{
   ...
   strpointers = xmalloc (sizeof(void *) * (numlines+3));
               ...
               char attribute, int srow, int scol, int flags, int help1)
{
    ...
    do
    {
        ...
        //video_put_n_char (M ("ú"), vis_length-l1, srow, scol+l);
        ...
        if (IS_KEY(k))
        {
            ...
            case _Enter :
                ...
                {
                    history[hist_num+1] = strcpy (xmalloc (strlen (buffer)+1), buffer);
                    hist_num++;
                }
                if (history[0] != NULL) free (history[0]);
            ...
            case _CtrlUp:
                ...
                {
                    if (history[0] != NULL) free (history[0]);
                    history[0] = strcpy (xmalloc (strlen (buffer)+1), buffer);
...
int localdir (directory *D, char *dirname)
{
    ...
    nfa = 64;
    D->files = xmalloc (sizeof(file) * nfa);
...
void lcache_scandir (char *dirname)
{
    ...
    if (lcache.lda == 0)
    {
        lcache.ld = 0;
        lcache.lda = 16;
        lcache.L = xmalloc (sizeof(directory) * lcache.lda);
    }
    if (lcache.lda == lcache.ld)
    {
        lcache.lda *= 2;
        lcache.L = xrealloc (lcache.L, sizeof(directory) * lcache.lda);
    }
