...
static int do_key_viewremote (int key, int action)
{
    ...
    case KEY_FM_MKDIR:
        if (is_http_proxy)
        {
            fly_ask_ok (0, M ("Not supported via HTTP proxy"));
            return ERR_FATAL;
        }
        make_remote_mkdir ();
        break;
    ...
    case KEY_FM_SAVE_LISTING:
        buffer[0] = '\0';
        if (entryfield (M ("Enter filename for a listing:"), 
            buffer, sizeof(buffer), 0) == PRESSED_ESC) break;
        save_listing (buffer);
        break;
    ...
    case KEY_FM_RENAME:
        ...
        if (entryfield (M ("Rename file:"), buffer, sizeof(buffer), 0) == PRESSED_ESC ||
            buffer[0] == 0) break;
        ...
        if (rsp != 2)
            fly_ask_ok (ASK_WARN, M ("Cannot rename '%s' to '%s'"), RCURFILE.name, buffer);
        else
            ask_refresh (buffer);
    ...
    case KEY_CHANGE_PERMISSIONS:
        ...
        {
            fly_ask_ok (0, M ("This FTP server does not support SITE CHMOD command"), NULL);
        }
        else
        {
            make_remote_chmod ();
    ...
    case KEY_FM_LOAD_DESCRIPTIONS:
        ...
        if (RCURFILE.flags & FILE_FLAG_DIR &&
            fly_ask_ok (0, M ("Cannot load descriptions from the directory '%s'.\n"
                              "Point the cursor to the ordinary file"), RCURFILE.name))
            break;
        if (!fly_ask (ASK_YN, M ("Load descriptions from '%s'?"), NULL, RCURFILE.name)) break;
        i = RCURFILENO;
    ...
    case KEY_FM_GO_ROOT:
        put_message (M ("Going to root directory on remote..."));
        change_dir ("/");
        break;
...
*/
    case KEY_FM_MASK_SELECT:
        ...
        {
            strcpy (buffer, "*");
            if (entryfield (M ("Enter filter:"), buffer, sizeof(buffer), 0) == PRESSED_ESC) break;
            for (i=0; i<RNFILES; i++)
            {
                if (markable(RFILE(i)))
                {
...
*/
    ...
    case KEY_FM_CHDIR:
        ...
        if (entryfield (M ("Change directory on remote to:"), 
            buffer, sizeof(buffer), 0) == PRESSED_ESC ||
            buffer[0] == '\0') break;
        put_message (M ("Changing directory on remote..."));
        if (options.new_transfer)
            r_chdir (buffer, 0);
        else
            change_dir (buffer);
    ...
    case KEY_FM_GO_UP:
        put_message (M ("Going to upper directory on remote..."));
        if (options.new_transfer)
            r_chdir ("..", 0);
        else
            change_dir ("..");
            ...
            if (site.system_type == SYS_MVS_NATIVE)
            {
                ...
                if (strcmp (RCURFILE.name, "..") == 0)
                {
                    ...
                    if (strchr (site.u.pathname, '.') == NULL)
                    {
                        dmsg ("MVS: at the top\n");
                        buffer[0] = '\0';
                        if (entryfield (M ("Change directory on remote to:"), buffer, sizeof(buffer), 0) == PRESSED_ESC ||
                            buffer[0] == 0) break;
                        put_message (M ("Changing directory on remote..."));
                        change_dir ("..");
                        change_dir (buffer);
...
static int do_key_viewlocal (int key, int action)
{
    ...
    case KEY_FM_CHDIR:
        ...
        if (entryfield (M ("Change local drive/directory to:"), buffer, sizeof(buffer), 0)
    ...
    case KEY_FM_MKDIR:
        buffer[0] = '\0';
        if (entryfield (M("Enter name of the directory to be created:"), 
            buffer, sizeof(buffer), 0) == PRESSED_ESC || buffer[0] == '\0') break;
#ifdef __WIN32__        
        rc = mkdir (buffer);
#else
        rc = mkdir (buffer, 0777);
#endif
        if (rc) fly_ask_ok (ASK_WARN, M ("Cannot create directory '%s'"), buffer);
    ...
    case KEY_FM_RENAME:
        ...
        strcpy (buffer2, LCURFILE.name);
        if (entryfield (M ("Rename file:"), buffer2, sizeof(buffer2), 0) == PRESSED_ESC ||
            buffer2[0] == 0) break;
        if (rename (LCURFILE.name, buffer2))
            fly_ask_ok (ASK_WARN, M("Cannot rename '%s' to '%s'"), LCURFILE.name, buffer2);
        else
        {
            build_local_filelist (NULL);
            try_set_local_cursor (buffer2);
    ...
    case KEY_FM_ENTER_DIR:
        ...
        else
        {
                ...
                (LCURFILE.rights & (S_IXUSR|S_IXGRP|S_IXOTH)))
            {
                ...
                str_cats (buffer, LCURFILE.name);
                strcat (buffer, " ");
                if (entryfield (M ("External command to execute:"), buffer, sizeof(buffer), 0) == PRESSED_ESC) break;
...
static int do_key_anymode (int key, int action)
{
    ...
    case KEY_FM_CHLOCAL_DRIVE:
        if (!fl_opt.has_driveletters)
        {
            fly_ask_ok (0, M ("Function is not supported on %s"), fl_opt.platform_name);
            break;
        }
        choose_local_drive ();
        build_local_filelist (NULL);
    ...
    case KEY_SYNC_BY_DOWNLOAD:
        if (!site.set_up) break;
        if (!fly_ask (ASK_YN, M("Synchronize local files with remote site?\n"
                                "\n"
                                "remote %s ->\n"
                                "local  %s\n"
                                "\n"
        ...
        rc = edit_mirror_options (&mo);
        if (rc != 0) break;
        set_mirror_options (&mo);
        rc = mirror (LOCAL_WITH_REMOTE, TRUE, &mo);
        if (rc == 1) fly_ask_ok (0, M("Directories seem to be identical"));
        break;

    case KEY_SYNC_BY_UPLOAD:
        if (!site.set_up) break;
        if (!fly_ask (ASK_YN, M("Synchronize remote site with local files?\n"
                                "\n"
                                "local  %s ->\n"
                                "remote %s\n"
        ...
        if (rc != 0) break;
        set_mirror_options (&mo);
        rc = mirror (REMOTE_WITH_LOCAL, TRUE, &mo);
        if (rc == 1) fly_ask_ok (0, M("Directories seem to be identical"));
        break;

    case KEY_SYNC_OPTIONS:
        get_mirror_options (&mo);
        rc = edit_mirror_options (&mo);
        if (rc != 0) break;
        set_mirror_options (&mo);
    ...
    case KEY_GEN_EXIT:
        if (!is_http_proxy && site.connected)
        {
            ...
            else
            {
                ...
                if (options.ask_exit)
                    if (!fly_ask (ASK_YN, M ("Exit NFTP?"), NULL)) break;
            }
        }
        else
        {
            if (options.ask_exit && !fly_ask (ASK_YN, M ("Exit NFTP?"), NULL)) break;
        }
        Logoff ();
        terminate ();
        exit (0);
    ...
    case KEY_SELECT_FONT:
        if (fl_opt.is_x11)
            put_message (M ("Enumerating fonts..."));
        if (F == NULL || nf <= 0)
        ...
        free (p);
        newfont = fly_choose (M ("Select screen font"), 0, &nf, startpos, drawline_select_font, NULL);
        if (newfont >= 0)
        {
    ...
    case KEY_GEN_OPEN_URL:
        ...
        if (entryfield (M ("Enter hostname[[:port]path] to login into:"), buffer, sizeof(buffer), 0) == PRESSED_ESC) break;
        if (buffer[0] == '\0') break;
        parse_url (buffer, &u);
        if (Login (&u)) break;
    ...
    case KEY_GEN_BOOKMARKS:
        ...
        //if (entryfield (M ("Select starting directory (press ENTER to accept):"), 
        //      u.pathname, sizeof(u.pathname), 0) == PRESSED_ESC) break;
        if (Login (&u)) break;
        if (options.login_bell) Bell (3);
        //if (site.connected) set_view_mode (VIEW_REMOTE);
        break;
    ...
    case KEY_INTERNAL_VERSION:
        fly_ask_ok (0,
                    ...
                    "Platform: %s\n"
                    "Copyright (C) Sergey Ayukov <asv@ayukov.com> 1994-2011\n",
    ...
    case KEY_GEN_SWITCH_PROXY:
        ...
        {
            fly_ask_ok (0, M ("No firewall/proxy has been configured in nftp.ini"));
            break;
        }
        if (!is_http_proxy && site.connected)
        {
        ...
        if (status.use_proxy && site.set_up)
        {
            ...
            if (strspn (site.u.hostname, " .0123456789") == strlen (site.u.hostname))
            {
                ...
                if (remote == NULL)
                {
                    PutLineIntoResp2 (M ("Cannot find '%s', trying by IP number"), site.u.hostname);
                    site.u.ip = addr;
                }
            }
            else
    ...
    case KEY_GEN_INFORMATION:
        ...
        {
            if (fly_ask (ASK_YN, M ("%d files in %d directories; %s bytes total\n\n"
        ...
        {
            fly_ask_ok (0, M ("%d files in %d directories; %s bytes total\n\n"
    ...
    case KEY_GEN_QUOTE:
        ...
        *buffer = 0;
        if (entryfield (M ("Enter command to be send verbatim:"), buffer, sizeof(buffer), 0)
            != PRESSED_ESC && buffer[0] != 0)
        {
            set_view_mode (VIEW_CONTROL);
            put_message (M ("Sending command to remote..."));
...
static void make_download_everything (void)
{
    ...
    if (marked_no == 0)
    {
        fly_ask_ok (0, M ("There are no marked files in the subdirectories"));
        return;
    }
    //if (!fly_ask (ASK_YN, M ("%d files in %d directories are marked for download.\n"
    //                         "%s bytes is the total size. Continue?\n"), NULL,
    //          marked_no, marked_dirs, pretty64(marked_size))) return;
    keep_tree = fly_ask (0, M("You have marked %d files in %d subdirectories.\n"
                              "Total size is %s bytes.\n"
                              "Preserve directory structure while transferring?"), 
                         M("Keep subdirectories\nPile up files"),
...
static void make_remote_delete (void)
{
    ...
    if (!fly_ask (ASK_YN|ASK_WARN, M ("Delete file(s)?"), NULL)) return;
...
static void make_remote_mkdir (void)
{
    ...
    buffer[0] = '\0';
    if (entryfield (M ("Enter name of the directory to be created:"), buffer, sizeof(buffer), 0) == PRESSED_ESC ||
        buffer[0] == '\0') return;

    put_message (M ("Creating directory on remote..."));
    ...
    if (rsp != 2)
        fly_ask_ok (ASK_WARN, M ("Cannot create directory '%s'"), dirname);
    else
        ask_refresh (buffer);
    display.quicksearch[0] = '\0';
}
...
static void make_remote_select (int walkonly, int movepointer)
{
    ...
    if (markable (RCURFILE) != 0 && RCURFILE.flags & FILE_FLAG_DIR)
    {
        ...
        else
        {
            ...
            if (!walked)
            {
                if (options.ask_walktree && !fly_ask (ASK_YN, 
                    M ("Walk entire tree starting from\n'%s'?"), NULL, buffer)) return;
                //oldmode = site.batch_mode;
                //site.batch_mode = TRUE;
...
static void make_download_byname (void)
{
    ...
    if (entryfield (M ("File to download:"), buffer, sizeof(buffer), 0) == PRESSED_ESC) return;
    str_translate (buffer, '\\', '/');
    p = strrchr (buffer, '/');
    if (p == NULL) p = buffer;
    else           p++;
    strcpy (buf2, p);
    if (entryfield (M ("Retrieve file as:"), buf2, sizeof(buf2), 0) == PRESSED_ESC
        || buffer[0] == '\0') return;
    oldmode = site.batch_mode;
...
static void make_view_remote (int browser)
{
    ...
    if (RCURFILE.cached == NULL)
    {
        if (RCURFILE.size > 512*1024 &&
            !fly_ask (ASK_YN|ASK_WARN, M ("The file '%s' is big -- %s bytes\n"
                                          "Do you still want to view it with internal viewer?"), 
                      NULL, RCURFILE.name, pretty64(RCURFILE.size))) return;
        rc = cache_file (site.cdir, RCURFILENO);
        //set_view_mode (VIEW_REMOTE);
        if (rc)
        {
            fly_ask_ok (ASK_WARN, M ("Transfer of '%s' has failed"), RCURFILE.name);
            return;
        }
    }
    ...
    else
    {
        ...
        if (rc1 == 0 && rc2 == 0 && before.st_mtime != after.st_mtime &&
            fly_ask (ASK_YN, M ("Upload modified file back to server?"), NULL))
        {
...
void ask_refresh (char *fn)
{
    ...
    {
        reread = fly_ask (ASK_YN, M ("Re-read remote directory contents?"), NULL, 0);
    }
...
int ask_logoff (void)
{
    ...
    if (marked_no)
    {
        return !fly_ask (ASK_YN, M ("You have marked %d file(s) in %d directories.\n"
                                    "Total size is %s bytes.\n"
                                    "Are you still sure you want to logoff? "), NULL,
                         marked_no, marked_dirs, pretty64(marked_size));
    }
    else if (options.ask_exit)
    {
        return !fly_ask (ASK_YN, M ("Logoff from %s?"), NULL, site.u.hostname);
    }
    else
    {
        return 0;
...
static void make_transfer (int direction)
{
    ...
    if (!mk)
    {
        ...
        if (entryfield (direction == DOWN ?
                        M ("Retrieve file as:") : M ("Upload file as:"),
    ...
    else
    {
        if (!fly_ask (ASK_YN, direction == DOWN ?
                      M ("Transfer file(s) FROM remote?") :
                      M ("Transfer file(s) TO remote?"), NULL)) return;
        oldmode = site.batch_mode;
        site.batch_mode = TRUE;
...
static void make_export (void)
{
    ...
    // ask confirmation
    buffer[0] = '\0';
    if (entryfield (M ("Enter filename to write the list to:"), 
        buffer, sizeof(buffer), 0) == PRESSED_ESC) return;

    // write file with URLs for wget
    urlfile = fopen (buffer, "w");
    if (urlfile == NULL)
    {
        fly_ask_ok (ASK_WARN, M (" Cannot open file '%s' for writing"), buffer);
        return;
    }
...
static void make_launch_wget (void)
{
    ...
    {
        fly_ask_ok (0, M ("You are trying to download with WGET from non-anonymous site.\n"
    ...
    if (marked_no > 0)
    {
        if (!fly_ask (ASK_YN, M ("Download %d files in %d directories;\n"
                                 "%s bytes total with external program?"), NULL, marked_no,
                      marked_dirs, pretty64(marked_size))) return;
    }
    else
    {
        if (!fly_ask (ASK_YN, M ("Download file %s with external program?"), \
            NULL, RCURFILE.name)) return;
    }
    ...
    if (entryfield (M ("External command to execute:"), buffer, sizeof(buffer), 0) == PRESSED_ESC) return;
...
static void make_select_language (void)
{
    ...
    selection = fly_choose (M ("Select user interface language"), 
          0, &n, startpos, drawline_select_language, NULL);
    if (selection >= 0)
    {
...
static void drawline_select_font (int n, int len, int shift, int pointer, int row, int col)
{
    ...
    if (fl_opt.platform == PLATFORM_OS2_PM)
        snprintf1 (buffer, sizeof(buffer), M ("width %3d, height %3d (%s)"), 
           F[n].cx, F[n].cy, F[n].name);
    else
...
static void make_remote_chmod (void)
{
    ...
    if (mk)
    {
        ...
        snprintf1 (buffer, sizeof(buffer), M ("Access rights for %d files"), mk);
    }
    else
    {
        strcpy (rights+1, perm_b2t (RCURFILE.rights));
        snprintf1 (buffer, sizeof(buffer), M ("Access rights for %s"), RCURFILE.name);
    }
    ...
    while (TRUE)
    {
        ...
        fly_ask_ok (ASK_WARN, M ("Invalid input:  [%s]\n"
                                 "Correct format: [rwxrwxrwx]"), rights+1);
        strcpy (rights+1, perm_b2t (RCURFILE.rights));
    }
    ...
    if (mk)
    {
        for (i=0; i<RNFILES; i++)
        {
            if (RFILE(i).flags & FILE_FLAG_MARKED)
            {
                ...
                if (rc == 0 && rsp == 5)
                {
                    fly_ask_ok (0, M ("This FTP server does not support SITE CHMOD command"));
...
void do_walk (int key)
{
    ...
    else if (display.view_mode == VIEW_REMOTE)
    {
        ...
        if (!walked)
        {
            //if (options.ask_walktree && !fly_ask (ASK_YN, M ("Walk entire tree starting from\n"
            //      "'%s'?"), NULL, buffer)) return;
            //oldmode = site.batch_mode;
            //site.batch_mode = TRUE;
...
void do_mark_by_mask (int key)
{
    ...
    strcpy (buffer, "*");
    if (entryfield (M ("Enter filter:"), buffer, sizeof(buffer), 0) == PRESSED_ESC) return;
...
void do_enter_dir (int key)
{
    ...
    if (display.view_mode == VIEW_LOCAL)
    {
                 ...
                 (LCURFILE.rights & (S_IXUSR|S_IXGRP|S_IXOTH)))
        {
            ...
            str_cats (buffer, LCURFILE.name);
            strcat (buffer, " ");
            if (entryfield (M ("External command to execute:"), 
                buffer, sizeof(buffer), 0) == PRESSED_ESC) return;
            fly_launch (buffer, TRUE, TRUE);
            /*
...
void do_local_delete (int key)
{
    ...
    //if (!fly_ask (ASK_YN|ASK_WARN, M ("Delete file(s)?"), NULL)) break;
    // check for selected files
    mk = 0;
    for (i=0; i<LNFILES; i++)
    ...
    if (mk)
    {
        if (LCURFILENO != LNFILES-1)
            ...
            strcpy (buffer2, LFILE(LCURFILENO-1).name);
        answer = fly_ask (ASK_YN, M ("You have selected %d file(s).\n"
                                     "Really delete them?"), NULL, mk);
    }
    else
    {
        if (LCURFILENO != LNFILES-1)
            ...
            strcpy (buffer2, LFILE(LCURFILENO-1).name);
        answer = fly_ask (ASK_YN, M ("Delete '%s'?"), NULL, LCURFILE.name);
    }
    if (answer == 0) return;
    // do the delete
    noisy = TRUE;
            ...
            if (LFILE(i).flags & FILE_FLAG_DIR)
            {
                if (rmdir (LFILE(i).name))
                {
                    if (errno == ENOTEMPTY || errno == EEXIST)
                    {
                        ...
                        if (!do_all_subdirs)
                        {
                            snprintf1 (buffer, sizeof(buffer), "%s\n%s\n  %s  ",
                                       fl_str.yes, fl_str.no, M("All"));
                            answer = fly_ask (ASK_WARN, M ("Directory '%s' is not empty.\n"
                                              "Delete entire tree?"), buffer, LFILE(i).name);
                            if (answer == 0) return;
                    ...
                    else
                    {
                        ...
                        if (noisy)
                            fly_ask_ok (ASK_WARN, M ("Cannot delete '%s'"), LFILE(i).name);
                    }
                }
            }
            else
    ...
    else
    {
        if (LCURFILE.flags & FILE_FLAG_DIR)
        {
            if (rmdir (LCURFILE.name))
            {
                if (errno == ENOTEMPTY || errno == EEXIST)
                {
                    if (fly_ask (ASK_YN|ASK_WARN, M ("Directory '%s' is not empty.\n"
                            "Delete entire tree?"), NULL, LCURFILE.name))
                ...
                {
                    if (noisy)
                        fly_ask_ok (ASK_WARN, M ("Cannot delete '%s'"), LCURFILE.name);
                }
            }
        }
        else
...
void do_remote_delete (int key)
{
    ...
    if (mk)
        ...
        answer = fly_ask (ASK_YN, M ("Delete '%s'?"), NULL, RCURFILE.name);
    if (answer == 0) return;

    // do the delete
    put_message (M ("Deleting %s..."), "");
    if (mk)
    {
        rc = 0;
        /* first we must delete files (they are in the current directory) */
                          ...
                          int *noisy_notempty, int *noisy_readonly)
{
    ...
    if (*noisy_notempty)
    {
        snprintf1 (answers, sizeof(answers),
                   "%s\n%s\n  %s  ", fl_str.yes, fl_str.no, M ("All"));
        a = fly_ask (ASK_WARN, M ("Directory '%s' is not empty.\n"
                                  "Delete entire tree?"), answers, path);
        if (a == 0 || a == 2)
        {
            free (path);
