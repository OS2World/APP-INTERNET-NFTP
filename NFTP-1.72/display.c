...
void update_split_list (void)
{
    ...
    if (!site.set_up)
    {
        lp = strlen (MSG(M_NOT_CONNECTED));
        video_put_str_attr (MSG(M_NOT_CONNECTED), lp, p_slf()+p_nlf()/2, slp+2+(video_hsize()-1-slp-2-lp)/2,
   ...
   case MODE_PARSED:
       ...
       if (abs(time_distance) > 6*30*24*60*60)
       {
           snprintf1 (buf, sizeof(buf), "%3s %2d  %4d %13s %c %s%s",
                    ...
                    tm1.tm_mday,
                    1900+tm1.tm_year,
                    LFILE(n).flags & FILE_FLAG_DIR ? MSG(M_DIR_SIGN) :
...
void display_bottom_status_line (void)
{
    ...
    case 1:
        ...
        if (display.view_mode == VIEW_REMOTE || display.view_mode == VIEW_CONTROL)
        {
            ...
            {
                snprintf1 (buf, sizeof(buf), MSG(M_WELCOME), NFTP_VERSION);
            }
        }
        if (display.view_mode == VIEW_LOCAL)
        {
    ...
    {
        snprintf1 (buf, sizeof(buf), MSG(M_QUICK_SEARCH), display.quicksearch);
...
void display_top_status_line (void)
{
    ...
    case VIEW_REMOTE:
        if (site.set_up)
        {
            ...
            snprintf1 (buf, sizeof(buf), MSG(M_STATUS), num_total,
                     pretty64(size_total),
                     status.binary ? MSG(M_STATUS_BIN) : MSG(M_STATUS_ASC),
                     num_marked, pretty64(size_marked));
        }
        else
        {
            strcpy (buf, MSG(M_NOT_CONNECTED));
        }
        break;
    ...
    case VIEW_CONTROL:
        memset (buf, ' ', sizeof (buf));
        snprintf1 (buf, sizeof(buf), MSG(M_CCLINES), cchistory.n);
    }
