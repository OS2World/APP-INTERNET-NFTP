...
#ifdef __MINGW32__
   #define wait_a_bit(a) sleep1 (a/1000000.0)
#else
...
RC transfer (int where, trans_item *T, int nt, int noisy)
{
    ...
    */
    ...
    for (i=0; i<nt; i++)
    {
        ...
        if (T[i].flags & TF_DIRECTORY)
        {
            if (where == DOWN)
            {
                ...
                if (rc)
                {
                    PutLineIntoResp2 (MSG(M_CANT_CREATE_DIRECTORY), LF);
                    goto skip;
                }
            }
            else if (where == UP)
        ...
        {
            PutLineIntoResp2 (MSG(M_CANT_CREATE_DIRECTORY), T[i].l_dir);
            goto further;
        }
    ...
    restart:
        ...
        {
            if (is_batchmode)
                PutLineIntoResp2 (MSG(M_CANT_OPEN_FOR_WRITING), LF);
            else
            {
                if (where == DOWN)
    ...
    retry_auth:
        ...
        case TR_SKIP:
            dmsg ("transfer: SKIP\n");
            PutLineIntoResp2 (MSG(M_RESP_TRANSFER_SKIPPED), RF);
            break;
            
        case TR_CANCEL:
            dmsg ("transfer: CANCEL\n");
            PutLineIntoResp2 (MSG(M_RESP_TRANSFER_CANCELLED), RF);
            break;
            
        default:
            dmsg ("transfer: ERROR\n");
            PutLineIntoResp2 (MSG(M_RESP_TRANSFER_ERROR), RF);
        }

    skip:
        PutLineIntoResp2 (MSG(M_RESP_TRANSFER_SKIPPED),
    ...
    PutLineIntoResp2 (MSG(M_RESP_TRANSFER_DONE), (unsigned long)site.speed);
...
static int open_handle (int where, int action, char *LF)
{
    ...
    if (hnd < 0)
    {
        ...
        PutLineIntoResp2 (MSG(M_RESP_CANT_OPEN), LF);
        return -1;
    }
...
static int issue_transfer_cmd (int where, int action, char *cmd, int64_t lsize, int *sock)
{
    ...
    if (is_http_proxy)
    {
        ...
        if (sock1 < 0) PutLineIntoResp2 (MSG(M_RESP_CANNOT_CONNECT));
        switch (sock1)
        {
        case ERR_FATAL:
    ...
    if (where == DOWN && action == ACT_REGET)
    {
        ...
        if (rsp != 3)
        {
            PutLineIntoResp2 (MSG(M_RESP_REGET_ISNT_SUPPORTED));
            socket_close (sock1);
    ...
    if (!status.passive_mode)
    {
        *sock = Accept (sock1);
        ...
        if (*sock < 0)
        {
            ...
            PutLineIntoResp2 (MSG(M_RESP_CONNECTION_NOT_ESTABLISHED));
            return *sock;
        }
    }
    else
                          ...
                          progress *cur, double *interval_start)
{
    ...
    if (tot->to.files == 1)
    {
        snprintf1 (line1, sizeof(line1), MSG(M_PERCENTAGE1), percent,
                 status.binary ? MSG(M_BINARY) : MSG(M_ASCII),
                 is_download ? MSG(M_DOWNLOAD) : MSG(M_UPLOAD));
    }
    else
    {
        snprintf1 (line1, sizeof(line1), MSG(M_PERCENTAGE2), percent,
                 status.binary ? MSG(M_BINARY) : MSG(M_ASCII),
                 is_download ? MSG(M_DOWNLOAD) : MSG(M_UPLOAD),
