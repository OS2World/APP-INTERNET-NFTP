...
enum {ACTION_NONE=0, ACTION_DOWNLOAD=1, ACTION_TESTONLY=2, ACTION_UPLOAD=3, ACTION_CMDLIST=4, ACTION_NICK_BOOK=5,
ACTION_NICK_HIST=6, ACTION_LOGIN=7, ACTION_OPEN_BOOKMARKS=8, ACTION_OPEN_HISTORY=9, ACTION_BADARGS=10};

static int     action, disc_after;
static char    *optarg1=NULL, *optarg2=NULL, *wintitle;
...
runtime_flags[] =
{
    ...
    {"show-owner",        FALSE, KEY_SHOW_OWNER,         &status.show_owner},
    ...
    {"binary",            TRUE,  KEY_BINARY_ASCII,       &status.binary},
    {NULL,                FALSE, KEY_PAUSE_TRANSFER,     &status.transfer_paused},
};

/* ------------------------------------------------------------- */

int mkdir1 (char *p, unsigned int mask)
{
#ifdef __MINGW32__
    return mkdir (p);
#else
...
void check_args (int argc, char *argv[])
{
    ...
    optind  = 1;
    opterr  = 0;
    while ((c = getopt (argc, argv, "f:F:g:G:p:P:h:bl:mctesB")) != -1)
    {
        switch (c)
        {
        case 'f':
    ...
    if (argv[optind] != NULL)
    {
        ...
        if (argv[optind+1] != NULL)
        {
            ...
            {
                // we don't allow more than two login sites in any case
                action = ACTION_BADARGS;
            }
            else
            {
...
int init (int argc, char *argv[])
{
    ...
    PutLineIntoResp (RT_RESP, 0, "Copyright (C) 1994--2000 Sergey Ayukov <asv@ayukov.com>");
    PutLineIntoResp (RT_RESP, 0, "Portions Copyright (C) Eric Young <eay@cryptsoft.com>");
...
void update_inifile (char *inifile, char *ifile)
{
    ...
    for (i=1; ; i++)
    {
        sprintf (backupname, /*sizeof(backupname),*/ "%s.%03u", inifile, i);
    ...
    if (rename (inifile, backupname) < 0)
    {
        ...
        for (i=1; ; i++)
        {
            sprintf (backupname, /*sizeof(backupname),*/ "nftp.%03u", i);
            if (access (backupname, F_OK) != 0) break;
        }
        if (rename (inifile, backupname) < 0)
        ...
        case ';':
            if (str_break_ini_line (buffer+1, &varname, &varvalue) == 0)
            {
                for (i=0; i<n_items; i++)
                    ...
                    {
                        sprintf (buffer, /*sizeof(buffer),*/ "%s = %s", varname, items[i].value);
                        //if (fl_opt.has_console)
...
void update_switches (void)
{
    ...
    {
        menu_set_switch (KEY_PASSIVE_MODE, status.passive_mode[display.view[display.cursor]]);
    }
}
