/*
 ...
 value; such file must have extension "-BFS-attributes-" by
 convention.
...
int write_attrfile (char *filename, befs_attr *attrs, int nattrs)
{
    ...
    for (i=0; i<nattrs; i++)
    {
        ...
        {
            ns = 'b';
            nr = base64_encode (attrs[i].name, strlen(attrs[i].name));
        }
        ...
        {
            vs = 'b';
            vr = base64_encode (attrs[i].value, attrs[i].length);
        }
        // actual output
