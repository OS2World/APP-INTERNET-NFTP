...
int history_add (url_t *u)
{
    ...
    history_write_line (hf, time(NULL), u->hostname, u->ip,
...
void history_do_clean (void)
{
    ...
    if (!options.slowlink)
        put_message (MSG(M_PROCESSING_HISTORY1), 1);
    t1 = clock1 ();
    for (i=0; i<nhist; i++)
    {
        if (!options.slowlink && i !=0 && i % 1000 == 0)
            put_message (MSG(M_PROCESSING_HISTORY2), 1, i);
    ...
    if (!options.slowlink)
        put_message (MSG(M_PROCESSING_HISTORY1), 2);
    t1 = clock1 ();
    for (i=0; i<nhist; i++)
    {
        if (hist[i].flags & HIST_DELETED) continue;
    ...
    if (!options.slowlink)
        put_message (MSG(M_PROCESSING_HISTORY1), 3);
    ...
    for (i=0; i<MAX_HIST_ENTRIES; i++)
    {
        rc = history_read_line
            (hf, &(hist[i].tm), hostname, &(hist[i].ip),
             userid, &(hist[i].port), &(hist[i].flags), directory);
        if (rc != 0) break;
        hist[i].hostname = strdup (hostname);
        if (userid[0] == '\0')
        {
            hist[i].userid = strdup ("");
        }
        else
        {
            hist[i].userid = strdup (userid);
        }
        hist[i].directory = strdup (directory);
        if (!options.slowlink && i != 0 && i % 1000 == 0 && clean)
            put_message (MSG(M_READING_HISTORY_FILE), i);
    }
    nhist = i;
    fclose (hf);
...
int history_write (void)
{
    ...
    for (i=0; i<nhist; i++)
    {
        ...
        if (!options.slowlink && i != 0 && i % 1000 == 0)
            put_message (MSG(M_WRITING_HISTORY_FILE), i);
    }
                         ...
                         char *userid, int port, int flags, char *directory)
{
    ...
    if (port != options.defaultport)
        sprintf (cport, /*sizeof(cport),*/ "%u", port);
    else
        strcpy (cport, "");

    fprintf (hf,
             "%lu,%s,%s,%s,%s,%s,,%s,\n",
             tm, hostname, inet_ntoa (in_addr1),
             cport, directory,
...
int history_nickname (char *nickname, url_t *u)
{
    ...
    {
        fly_ask_ok (0, MSG(M_ERROR_READING_HISTORY), options.history_file);
        return 0;
    }

    if (nhist == 0)
    {
        fly_ask_ok (0, MSG(M_EMPTY_HISTORY), options.history_file);
        return 0;
    }
    ...
    if (nchoices == 0)
        fly_ask_ok (ASK_WARN, MSG(M_HISTORY_NICKNAME), nickname);

    // found multiple choices?
    if (nchoices > 0)
        nch = fly_choose (MSG(M_SELECT_HISTORY), CHOOSE_ALLOW_HSCROLL,
...
void draw_hist_line (int n, int len, int shift, int pointer, int row, int col)
{
    ...
    num = choices[n];
    sprintf (buffer, " %s:%s ", hist[num].hostname, hist[num].directory);
    l = strlen (buffer);
    if (shift < l)
...
int history_select (url_t *u)
{
    ...
    do
    {
        if (view == 2)
        {
            sprintf (buf, /*sizeof(buf),*/ MSG(M_HIST_HEADER1), nsites, nhist);
        ...
        {
            sprintf (buf, /*sizeof(buf),*/ MSG(M_HIST_HEADER2), sites[f_cursor].hostname,
                     sites[f_cursor].N);
    ...
    while (selection == -2);
    update (1);
    if (changed && fly_ask (ASK_YN, MSG(M_SAVE_HISTORY), NULL))
        history_write ();
...
void drawline_site (int n, int len, int shift, int pointer, int row, int col)
{
    ...
    tmdt = gmtime (&(hist[sites[n].n[0]].tm));
    sprintf (buf, /*sizeof(buf),*/ " %02u/%02u/%04u %02u:%02u  %s (%s) ",
...
void drawline_dir (int n, int len, int shift, int pointer, int row, int col)
{
    ...
    tmdt = gmtime (&(hist[ni].tm));
    if (hist[ni].port != options.defaultport)
        sprintf (buf, /*sizeof(buf),*/ "%02u/%02u/%04u %02u:%02u %s %s :%u%s",
                 tmdt->tm_mday, tmdt->tm_mon+1, 1900+tmdt->tm_year,
                 tmdt->tm_hour, tmdt->tm_min,
                 hist[ni].userid,
        ...
        sprintf (buf, /*sizeof(buf),*/ "%02u/%02u/%04u %02u:%02u %s %s %s",
                 tmdt->tm_mday, tmdt->tm_mon+1, 1900+tmdt->tm_year,
                 tmdt->tm_hour, tmdt->tm_min,
                 hist[ni].userid,
                 makeflags (hist[ni].flags),
...
int  callback_site (int *nchoices, int *n, int key)
{
    ...
    if (IS_KEY (key))
    {
        ...
        case _Delete:
            if (!fly_ask (ASK_YN|ASK_WARN, MSG(M_HIST_DELETE_SITE), NULL,
        ...
        ask_for_target:
            ...
            buffer = malloc (1024);
            buffer[0] = '\0';
            rc = entryfield (MSG(M_SEARCH_FORWARD), buffer, 1024, 0);
        ...
        case _CtrlG:
            ...
            if (i == nsites)
                fly_ask (0, MSG(M_CANT_FIND), NULL, buffer);
            else
                *n = i;
            break;
        ...
        case _CtrlH:
            if (buffer == NULL || buffer[0] == '\0')
            {
                ...
                buffer = malloc (1024);
                buffer[0] = '\0';
                rc = entryfield (MSG(M_SEARCH_BACKWARDS), buffer, 1024, 0);
                buffer = realloc (buffer, strlen (buffer)+1);
                if (rc != PRESSED_ENTER || buffer[0] == '\0') break;
            }
            for (i=*n-1; i>=0; i--)
...
int  callback_dir  (int *nchoices, int *n, int key)
{
    ...
    if (IS_KEY (key))
    {
        ...
        case _Delete:
            if (!fly_ask (ASK_YN, MSG(M_HIST_DELETE_DIR), NULL, sites[f_cursor].hostname,
        ...
        case _CtrlG:
            ...
            {
                if (!fly_ask (ASK_YN, MSG(M_HIST_CONT_SEARCH), NULL, sites[f_cursor].hostname)) break;
